...
internal static class ArrowUtil
{
    public static Point ComputeLocation(FsmNodeData node1, FsmNodeData node2, float yPos, out bool isLeft)
    {
        ...
        isLeft = midx1 == midx2
|| (Math.Abs(midx1 - midx2) * 2 < nodetfm1.Width + nodetfm2.Width ? midy2 > midy1 ? midx1 < midx2 : midx1 > midx2 : midx1 < midx2);

        loc = isLeft ? new Point(loc.X + (nodetfm1.Width / 2), loc.Y) : new Point(loc.X - (nodetfm1.Width / 2), loc.Y);
